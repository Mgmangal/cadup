...
class AaiController extends Controller
{
    ...
    public function flyingLogList(Request $request)
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $sub_array[] = is_set_time_format($value->departure_time).' / '. is_set_time_format($value->arrival_time);
    ...
    public function generate($id)
    {
        ...
        // return $data;
        $flightDocAssign = FlightDocAssign::whereJsonContains('flying_logs', "$id")->first();
        
        return view('aai_reports.generate', compact('data','flightDocAssign'));
    }
    ...
    {   
        ...
        $model->booking_date =Carbon::createFromFormat('d-m-Y h:i',$request->booking_date)->format('Y-m-d H:i:s');
        $model->modification_date = Carbon::createFromFormat('d-m-Y h:i',$request->modification_date)->format('Y-m-d H:i:s');
        $model->original_pnr = $request->original_pnr;
        $model->parent_pnr = $request->parent_pnr;
        $model->tail_number = $request->tail_number;
        $model->departure_date = Carbon::createFromFormat('d-m-Y h:i',$request->departure_date)->format('Y-m-d H:i:s'); 
        //is_set_date_time_format($request->departure_date);
        $model->departure_date_utc = Carbon::createFromFormat('d-m-Y h:i',$request->departure_date_utc)->format('Y-m-d H:i:s'); 
        //is_set_date_time_format($request->departure_date_utc);
        $model->departure_date_local = Carbon::createFromFormat('d-m-Y h:i',$request->departure_date_local)->format('Y-m-d H:i:s');
        //is_set_date_time_format($request->departure_date_local);
        ...
        $model->adult_count = $request->total_pax;
        ...
        $model->pnr_status = $request->pnr_status;//($request->pnr_status > 0) ? 'Borded' : 'No show';
        $model->save();
    ...
    public function list(Request $request)
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $FlyingLog=FlyingLog::find($value->flying_log_id);
            $sub_array = array();
            $sub_array[] = ++$key;
            // $sub_array[] = $value->flying_log_id;
            $sub_array[] = $value->d_i_ind;
            $sub_array[] = $value->rcs_ind;
            $sub_array[] =($FlyingLog->departure_time!=null ? Carbon::parse(is_get_date_time_format($FlyingLog->departure_time),'UTC')->setTimezone('Asia/Kolkata')->format('d-m-Y H:i'): '');
            // $sub_array[] = Carbon::parse($value->booking_date)->setTimezone('Asia/Kolkata')->format('Y/m/d h:i:s')
            //$sub_array[] = is_get_date_time_format($value->booking_date);
            // $sub_array[] = Carbon::parse(is_get_date_time_format($value->modification_date),'Asia/Kolkata')->format('Y/m/d H:i:s');
            // $sub_array[] = Carbon::parse($value->modification_date)->setTimezone('Asia/Kolkata')->format('Y/m/d h:i:s');
            ...
            // departure_date
            $sub_array[] =($FlyingLog->departure_time!=null ? Carbon::parse(is_get_date_time_format($FlyingLog->departure_time),'UTC')->setTimezone('Asia/Kolkata')->format('d-m-Y H:i'): '');
            //$sub_array[] = is_get_date_time_format($value->departure_date);
            
            // departure_date_utc
            $sub_array[] =($FlyingLog->departure_time!=null ? Carbon::parse(is_get_date_time_format($FlyingLog->departure_time),'UTC')->format('d-m-Y H:i:s'): '');
            
            //$sub_array[] = $value->departure_date_utc ? Carbon::parse($value->departure_date_utc,'Asia/Kolkata')->setTimezone('UTC')->toDateTimeString() : '';
            
            // departure_date_local
            $sub_array[] =($FlyingLog->departure_time!=null ? Carbon::parse(is_get_date_time_format($FlyingLog->departure_time),'UTC')->setTimezone('Asia/Kolkata')->format('d-m-Y H:i'): '');
            //$sub_array[] = is_get_date_time_format($value->departure_date_local);
    ...
    public function bulkStore(Request $request)
    {
        ...
        if ($flyingLogsWithoutReports->isNotEmpty())
        {
            ...
            $flightDocAssign = FlightDocAssign::whereJsonContains('flying_logs', "$flyingLog->id")->first();
            
            $departureTime = Carbon::parse($flyingLog->departure_time, 'UTC')->setTimezone('Asia/Kolkata');
            $formattedDateTime = $departureTime->format('d-m-Y H:i');
            $formattedDateTimeForDatabase = $departureTime->format('Y-m-d H:i:s');
            $formattedDateOnly = $departureTime->format('dmY');
            
            $model = new AaiReport();
            $model->flying_log_id = $flyingLog->id;
            $model->d_i_ind = 'D';
            $model->rcs_ind = 'Non RCS';
            $model->booking_date = $formattedDateTimeForDatabase;
            $model->modification_date = $formattedDateTimeForDatabase;
            
            $callSign = str_replace('-', '', $flyingLog->aircraft->call_sign);
            $model->original_pnr = $formattedDateOnly . $callSign . $departureTime->format('Hi');
            $model->parent_pnr = $formattedDateOnly . $callSign . $departureTime->format('Hi');
            
            $model->tail_number = $flyingLog->aircraft->call_sign;
            $model->departure_date = $formattedDateTimeForDatabase;
            $model->departure_date_utc = $departureTime->copy()->setTimezone('UTC')->format('Y-m-d H:i:s');
            $model->departure_date_local = $formattedDateTimeForDatabase;
            ...
            $total_pax = $flyingLog->passenger;
            $model->total_pax = $total_pax;
            $model->adult_count = $total_pax;
            $model->child_count = '0';
            ...
            $model->pnr_status = ($total_pax > 0) ? 'Borded' : 'No show';
